...
public class Utils {
	...
	public static void performJobSanityChecks(Job job) {
		...
		pt = (ParameterizedType) newJob.getReducerClass().getGenericInterfaces()[0];
		...
		try {
			// Check if mapper input types match that required by file input format
			InputFormat<?,?> format = (InputFormat<?, ?>) job.getFileInputFormatClass().newInstance();
			if (!(format.getKeyType().equals(mapK1) && format.getValueType().equals(mapV1))) {
		...
		// Check if the types user specified are the same as the actual mapper and reducer
		if (!(mapK2.equals(newJob.getMapperOutputKeyClass().getName()))) {
			throw new IllegalArgumentException("Actual Mapper output key type and that provided in configuration don't match. Expected: " + mapK2 + "; Actual: " + "lib." + newJob.getMapperOutputKeyClass().getName());
		}
		if (!(mapV2.equals(newJob.getMapperOutputValueClass().getName()))) {
			throw new IllegalArgumentException("Actual Mapper output value type and that provided in configuration don't match");
		}
		if (!(reducerK2.equals(newJob.getReducerOutputKeyClass().getName()))) {
			throw new IllegalArgumentException("Actual Reducer output key type and that provided in configuration don't match");
		}
		if (!(reducerV2.equals(newJob.getReducerOutputValueClass().getName()))) {
			throw new IllegalArgumentException("Actual Reducer output value type and that provided in configuration don't match");
		}
		
		// Check if output types of mapper are same as input types of reducer
		if (!(mapK2.equals(reducerK1))) {
			throw new IllegalArgumentException("Mapper output key type does not match reducer input key type");
		}
		if (!(mapV2.equals(reducerV1))) {
			throw new IllegalArgumentException("Mapper output value type does not match reducer input value type");
		}
		...
		if (mapK2.equals(NullWritable.class.getName()) ||
			...
			throw new IllegalArgumentException("Input/output key/value types (except Mapper input key type) cannot be NullWritable");
		}
	}
